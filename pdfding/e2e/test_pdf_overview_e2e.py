...
class NoPdfE2ETestCase(PdfDingE2ETestCase):
    def test_pdf_overview_no_pdfs(self):
        with sync_playwright() as p:
            ...
            expect(self.page.locator("body")).to_contain_text("Collection has no PDFs yet")
...
class PdfOverviewE2ETestCase(PdfDingE2ETestCase):
    ...
    def test_sort(self):
        ...
        pdf = self.user.profile.current_pdfs.get(name='pdf_2_12')
        pdf.views = 1001
        pdf.save()

        pdf = self.user.profile.current_pdfs.get(name='pdf_3_13')
        pdf.views = 101
        pdf.save()
